...
EMBEDDING_MODEL = "text-embedding-3-small"     # OpenAI 임베딩

pdf_path    = "tests/한국가스공사_지진감지시스템 규격표준_20260506.pdf"
safe_name   = EMBEDDING_MODEL.replace("/", "_").replace(".", "-")
...
def get_answer(groq_client, context: str, question: str) -> str:
...
def main():
    ...
    try:
        for i, qa in enumerate(QA_DATASET, start=1):
            ...
            answer  = get_answer(groq_client, context, question)
