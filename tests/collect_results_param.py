...
# ─────────────────────────────────────────────
# 파라미터 실험 설정 (여기만 바꾸면 됨)
# ─────────────────────────────────────────────
LLM_MODEL       = "llama-3.1-8b-instant"                                    # LLM 고정
EMBEDDING_MODEL = "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2"  # 임베딩 고정

# 실험 1: 가중치 변경 (60/40 → 50/50)
FAISS_WEIGHT = 0.5   # 기본값 0.6 → 0.5로 변경
BM25_WEIGHT  = 0.5   # 기본값 0.4 → 0.5로 변경

# 실험 2: 청크 크기 변경은 config.py에서 DEFAULT_CHUNK_SIZE 수정

K_VALUE   = 5     # k값 고정
DELAY_SEC = 10    # Groq 토큰 한도 방지 딜레이

PARAM_NAME = "chunk_500"  # weight_50_50 → chunk_500
pdf_path    = "tests/한국가스공사_지진감지시스템 규격표준_20260506.pdf"
output_path = f"tests/results_param_{PARAM_NAME}.json"
...
def get_retriever(vectorstore, split_docs):
    ...
    return EnsembleRetriever(
        ...
        weights=[FAISS_WEIGHT, BM25_WEIGHT]  # 파라미터 실험 가중치
    )
...
def main():
    ...
    print(f"파라미터: FAISS={FAISS_WEIGHT}, BM25={BM25_WEIGHT}, k={K_VALUE}")
    print("PDF 로딩 중...")
